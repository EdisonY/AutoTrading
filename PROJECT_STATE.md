...
- 2026-05-31 operational note: the regular Tencent→Aliyun shadow sync can still hit timeouts when the live DB is large or the transfer path is weak. In that case, use a bounded SQLite slim/tiny snapshot (recent `events`, latest `sentinel_scans`, latest `account_snapshots`, attention tables), verify `pragma quick_check`, transfer with bounded SSH/base64 or another streaming-safe method, then regenerate research-store, counterfactual, portal, and sync only key report/runtime outputs. This is a workaround until `shadow_sync_from_tencent.py` is made fully incremental/streaming-safe.
