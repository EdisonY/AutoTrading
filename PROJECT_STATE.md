...
- 2026-05-31 update: Tencent→Aliyun shadow sync now defaults to a bounded SQLite mirror: recent `events`, newest `sentinel_scans`, newest `account_snapshots`, and attention tables, followed by `pragma quick_check` and freshness validation. Aliyun→Tencent report sync now sends key runtime/report outputs first and keeps bulky market-review files optional.
