...
def _get_exchange_info_cached() -> dict:
    ...
    if not scanner_binance_public_fallback_enabled():
        logger.warning("Binance exchangeInfo fallback disabled; skip B/v16 market metadata fetch")
...
def _request(method: str, path: str, params: dict = None) -> dict:
    ...
    if api_queue_client_enabled():
        ...
        return data
    wait_before_request("B/v16", method, path)
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            if isinstance(data, dict) and data.get("code") is not None and str(data.get("code")) != "200":
                ...
                record_response("B/v16", method, path, data.get("code"), json.dumps(data, ensure_ascii=False))
            return data
    except urllib.error.HTTPError as e:
        body = e.read().decode("utf-8", errors="replace")
        logger.error(f"HTTP {e.code}: {body[:500]}")
        record_response("B/v16", method, path, e.code, body)
...
def _public_request(path: str) -> dict:
    ...
    try:
        if api_queue_client_enabled():
            return queued_api_request(scope="public", label="B/v16-client", method="GET", path=path)
...
class BinanceClientV2:
    ...
    def close_position(self, symbol: str, pos_side: str, quantity: float = 0, order_side: str = "", position_side: str = "") -> dict:
        ...
        if quantity > 0:
            params["quantity"] = format_decimal_down(quantity, _get_step_size(symbol)[0], 8)
