...
MAX_POS_PER_SECTOR = 2              # P2: 同赛道限制更硬
...
class Scanner:
    ...
    def _passes_15m_confirmation(self, sym: str, side: str) -> tuple[bool, str]:
        sig = self.strategy_engine.analyze(sym, CONFIRM_TIMEFRAME)
        if not sig or not sig.get("can_trade"):
    ...
    def scan_cycle(self):
        ...
        for tf in ENTRY_TIMEFRAMES:
            for sig in all_signals[tf]:
                ...
                confirmed, confirm_reason = self._passes_15m_confirmation(sym, side)
                if not confirmed:
