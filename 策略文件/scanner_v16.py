...
def fetch_klines(symbol, bar="15m", limit=100):
    ...
    if cached:
        return cached
    url = f"https://testnet.binancefuture.com/fapi/v1/klines?symbol={symbol}&interval={bar}&limit={limit}"
    try:
        raw = fetch_json(url)
        if not raw: return []
        rows = []
...
class ScannerV16:
    ...
    def _open_position(self, tf, sym, sig, side, score, confirm_reason=""):
        ...
        if not self._can_open_new_position(TRADE_SIZE, tf, sym, side, score):
            return False
    ...
    def _enforce_hard_stop_on_exchange(self):
        ...
        for p in raw_positions:
            ...
            lev = float(p.get("leverage", LEVERAGE) or LEVERAGE)
