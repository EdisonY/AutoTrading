...
- [x] L1 预注册 regime-specific validation：`l1_regime_specific_validation.py` 已落地并跑完两年本地 `30m/1h/4h`。它先验测试极端波动跳变、range-chop pump/dump fade、压缩突破/跌破、breadth crack/thrust，不事后围绕 K2 调参。结果 `84144` sampled events、`1350876` records、`edge_candidate=2/watchlist=56/rejected=62`。两个 forward edge 是 `L1_high_vol_negative_jump_bounce_long / 1h / 12bar`（uplift avg `+0.318887%`、win uplift `+3.386%`）和 `L1_range_chop_pump_fade_short / 1h / 3bar`（uplift avg `+0.105971%`、win uplift `+3.119%`）。
- [x] L1 完整策略重构：`l1_edge_strategy_reconstruction.py` 已落地并跑完两年本地全量。10 个预注册 1h 变体结果 `near_miss=3/rejected=7/research_candidate=0`。最佳 robust row `l1_neg_jump_12bar_time` 全样本 `+920.765112 USDT`、PF `1.193060`、`2924` 笔，但 validation/test PF 均低于 `1.10`；最佳 full PnL row `l1_neg_jump_24bar_slow_time` 全样本 `+1628.129342 USDT`、PF `1.305714`，但 validation `-191.323670`、PF `0.831209`。结论：L1 只保留负跳反弹结构线索，不进 paper/live/research_candidate。
- [x] L2 微结构短窗验证框架：`l2_microstructure_short_window.py` 已落地并首跑完成。它只用本地已有真实 compact OFI/CVD/depth/spread 样本，对齐本地 Kline 短窗 forward return，并和同币种 matched random controls 比较。首跑 `micro_events=140`、`depth_events=50`、`classified_events=59`、`aligned_records=180`、`aligned_symbols=10`，所有 `45` 个 signal/interval/horizon row 都 rejected，`microstructure_candidate=0`。结论是 data gap：工具可用，但真实微结构历史不足，不能从两年 Kline 仓伪造盘口证据。
- [x] 候选 x A/B 匹配矩阵：`candidate_ab_match_research.py` 已落地并跑完两年 clean27 本地全量。它把 L1 负跳反弹、E/4h 横截面强弱、J3 压缩突破分别做 standalone、A/v11 同向叠加、B/v16 同向叠加、A/B 反向 veto。结果：`7` 个候选变体里，`2` 个 E/4h standalone 达到 `manual_review_for_independent_strategy_id`，`5` 个为 watch/reconstruction，`0` 个 A/B 过滤器通过。E/4h 两个通过行：`lookback=48,top=3,hold=8` 全样本 `+2055.015189 USDT`、PF `1.839034`、`540` 笔、validation/test 均正；`lookback=48,top=5,hold=8` 全样本 `+2216.143310 USDT`、PF `1.711946`、`540` 笔、validation/test 均正。L1 与 J3 不能接 A/B paper；所有 A/B filter 都只是 watch hint 或 rejected。下一步只能为 E/4h 另起独立 `strategy_id` 的 paper-adapter 设计和风险规格；不能把 E 当 A/B filter，也不能自动接入账本。
...
- [ ] 下一轮并行记录：J3 暂停参数拟合，只保留 range-chop/突破结构线索；X7/K2/L1/L2 只保留结构线索或数据缺口；4h breadth watchlist、K2 `high_vol_chop_v2`、L1 negative-jump bounce、L2 depth/OFI 小样本 uplift 只能作为后续预注册 context/filter clue，不能直接 paper/live。
- [ ] 下一阶段 E/4h 独立策略 paper 设计：只在本地先写清 portfolio 架构、重平衡频率、最大腿数、单腿 notional、总风险、A/B 账本隔离字段、report 展示和停止条件。通过风险规格后再考虑服务器 paper 采样；自动调参/回滚/升级仍关闭。
- [ ] 下一阶段主线 L2 采样：继续真实 compact OFI/CVD/depth/spread/impact 样本采集，目标至少 `>=50` 条同一 signal/interval/horizon aligned records 且 train/validation/test 各有足够样本；历史不足就承认短窗研究，不从 Kline 伪造盘口。
...
- [x] Post-launch staged expansion layer 1: after clean post-cooldown restoration, Tencent B/v16 and C/v14 were raised from `1+1` scanner universe to `3+3` while keeping Kline network disabled and scanner market-data network disabled. Verification showed queue empty, cooldown empty, latest public rows `200`, no fresh `418/429/-1003`, and fresh B/C scanner rows. Next expansion layer is post-launch observation work, not a first-version blocker, and must remain gated by clean queue/journal checks.
- [x] Post-launch staged expansion layer 2: after a clean `3+3` window, B/v16 and C/v14 were raised to `5+5`. Scan frequency remained unchanged; expansion uses wider universe first, not faster API cadence. Kline network and scanner market-data network remain disabled, and first post-change observation stayed clean.
- [x] Post-launch Top100 volume universe restoration: user clarified the intended universe is Top100 by trading volume or market cap. Existing scanner input supports Binance trading-volume Top100, so A/B/C live drop-ins now target Top100 volume coverage while keeping frequency unchanged and Kline/network fallbacks disabled. Market-cap Top100 source integration remains post-launch optimization.
- [x] Post-launch Top100 effective coverage release: after the first Top100 restoration still produced no opens and B/C were only scanning about 45 symbols / 23 Top100 symbols, A/B/C scanner `SENTINEL_LIMIT` was raised to `100` so each strategy can effectively scan the Top100 volume universe. Frequency stayed at `120s`, cache/sentinel stayed at `300s`, Kline direct network stayed off, scanner market-data fallback stayed off, and strategy thresholds/sizing/leverage/stops were unchanged. Immediate observation showed A/B/C real scans fresh and queue/cooldown/recent_bad all clean; open evidence still needs natural signals.
- [x] Post-launch external market-data pressure split: B/v16 first moved Kline/OFI/funding/CVD to OKX read-only data and proved Binance queue pressure dropped with no B/v16 public queue label. A/v11 and C/v14 then moved Kline reads to the same cache -> OKX -> original-path order. Tencent A/B/C now have OKX enabled while Binance direct Kline and scanner market-data fallback stay off. Post-enable observation showed queue/cooldown/bad all clean, fresh Kline cache growth, and A/B/C Top100+ scanning. Frequency remains unchanged; next cadence work is a separate one-strategy clean-window test.
- [x] Post-launch scanner cadence control: A/v11, B/v16, and C/v14 now support env-controlled scanner intervals so frequency can be adjusted by systemd drop-in rather than code edit. The first all-strategy release target is `90s` with OKX per-process limit lowered to `60/min`; Binance Kline and scanner market-data fallback remain disabled. Rollback path is restoring interval env values to `120s`.
- [x] Post-launch external-data hot-path hardening: after the 90s release, no-open diagnosis showed B/v16/C/v14 were mostly blocked by existing strategy gates, while A/v11 had scanner freshness risk from repeated OKX/cache misses and unsupported symbols. OKX helper now fails fast when per-process budget is exhausted, rejects unsupported/non-ASCII symbols before network calls, and negative-caches concrete external lookup failures. This is scanner stability work only; thresholds, sizing, leverage, stops, Binance direct Kline, and scanner market-data fallback remain unchanged.
- [x] Post-launch A/v11 heartbeat repair: A/v11 scanner liveness is now visible through `SCAN_STATS` system events, because `sentinel_scans` only advances for rows with sentinel context and can falsely look stale during cache-only/no-sentinel windows. A now emits `phase=start` at scan-cycle start, `phase=complete` at normal end, and `phase=error` for symbol-list failures, so report freshness does not wait for a long Top100+ scan to finish. OKX empty market-data responses are negative-cached, and A/v11 VPB funding-rate lookup prefers OKX read-only funding; when OKX market data is enabled, Binance public `premiumIndex` fallback is disabled by default unless `VPB_BINANCE_FUNDING_FALLBACK_ENABLED=1`. No strategy parameter, Binance Kline switch, sizing, leverage, stop, order enablement, interval, or universe-size change.
- [x] Post-launch scanner Binance public fallback closure: A/B/C scanner-side Binance public market fallback now defaults off through `SCANNER_BINANCE_PUBLIC_FALLBACK_ENABLED=0`. B/v16 OFI/depth, funding, and CVD/trades use OKX first and otherwise return neutral/empty values instead of Binance depth/funding/aggTrades. A/v11 and C/v14 current-price helpers use local Kline cache latest close first and otherwise fail closed/skip price checks instead of Binance ticker-price. Central market-data-cache/sentinel remain the paced public ticker producers; per-strategy direct public fallback should stay opt-in only. Tencent A/B/C releases `20260606-112601-strategy-a-fdf046b`, `20260606-112657-strategy-b-fdf046b`, and `20260606-112744-strategy-c-fdf046b` deployed this rule; post-deploy queue check was clean with no active cooldown/request and no fresh `418/429/-1003`.
- [x] Waiting-period no-pressure optimization report: `waiting_period_optimization.py` now gives a report-only/offline work surface for periods where Binance pressure must not increase. It reads only local/mirrored runtime artifacts and shows queue/cooldown, Top100 volume coverage, no-open reasons, Kline/depth plan-only gaps, report freshness, and safe waiting actions. It is wired into Aliyun refresh/shadow, reverse sync, live-context pull, release bundles, decision portal, full portal, and tests. No Binance request, queue submit, service restart, scanner frequency change, or strategy parameter change.
- [x] Waiting-period next-stage attribution and freshness guard: `waiting_period_optimization.py` now adds plain-Chinese no-open attribution, SCAN_STATS reason summaries, live A/B/C heartbeat rows, Top100 cache parsing from `available_symbols`, and a stale-mirror guard for real-scan coverage. If the local/mirrored event DB is older than the selected window, the report says coverage is unknown instead of showing a misleading zero. Readiness remains `hold_frequency` / `ready_for_plan_only_data_work`; Kline/depth submit, scanner frequency increase, service restart, and strategy parameter changes remain disabled.
- [x] Operator cockpit report polish: `decision_portal.py` first screen now shows Top100 real-scan state and a small-release gate in a cockpit-style layout; `waiting_period_optimization.py` detail HTML now uses cards, progress bars, heartbeat, coverage, no-open reason, and safety-gate sections instead of a plain Markdown dump. This is UI/report-only and does not change scanner frequency, Kline/depth submit, queue behavior, or strategy parameters.
- [x] Fresh-start dirty-data reset for paper-exchange first version: Tencent reset archive `/opt/crypto-auto-trader/archive/testnet_data_reset/20260606-210624`; old `events/sentinel_scans/account_snapshots` reset to `0/0/0` while attention/baseline remain; fresh paper exchange created A/B/C `5/5/5` new OPEN rows. Replay/evolution remains waiting for closed-loop samples, not just holding/open samples.
