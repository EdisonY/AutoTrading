...
- [ ] Remaining first-version hard gates: P0-A staged clean run, then final dirty-data archive/reset and fresh zero-run after reset pre-checks show queue/services safe. P0-B open/close natural exact rows move to post-launch evidence collection and must not block first-version skeleton landing.
