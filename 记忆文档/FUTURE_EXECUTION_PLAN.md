...
2026-06-04 冷却后恢复规则补充：B/v16 signed baseline 触发的 `HTTP 418/-1003` cooldown 已在 `2026-06-04 15:15:54.959 +08` 后清除，read-only queue/service/journal check clean。当前已只恢复 API queue、market-data-cache、sentinel、A/v11 user-stream；queue rows `486`-`496` 全部 `done/200`，A listenKey row `494` 为 `200`。B/C user-stream、A/B/C scanners、account-snapshot 仍停。不要重试 B/C signed baseline；下一步按 B user-stream 单步启动、观察、C user-stream 单步启动、观察、scanner staged 的顺序推进。`binance_start_guard.py` 继续作为本地 `ExecStartPre` 安全带，只读 queue cooldown DB，不调用 Binance；它不替代每步前后的 queue/service/journal clean check。
...
- [ ] Post-launch staged restoration layer 3a: post-cooldown clean check passed and only API queue, cache, sentinel, and A/v11 user-stream are restored. B/C signed baseline must not be retried. Next: start B/v16 user-stream alone, observe queue/cooldown/journals, then start C/v14 user-stream alone if clean.
