...
2026-06-04 冷却后恢复规则补充：B/v16 signed baseline 触发的 `HTTP 418/-1003` cooldown 已在 `2026-06-04 15:15:54.959 +08` 后清除，read-only queue/service/journal check clean。当前已只恢复 API queue、market-data-cache、sentinel、A/v11 user-stream、B/v16 user-stream；queue rows `486`-`503` 全部 `done/200`，A listenKey row `494` 与 B listenKey row `501` 均为 `200`。C user-stream、A/B/C scanners、account-snapshot 仍停。不要重试 B/C signed baseline；下一步按 C user-stream 单步启动、观察、scanner staged 前账户状态策略检查的顺序推进。`binance_start_guard.py` 继续作为本地 `ExecStartPre` 安全带，只读 queue cooldown DB，不调用 Binance；它不替代每步前后的 queue/service/journal clean check。
...
1. **P0-A Binance API 根治**
   ...
   - 未完成：normal scanner universe、长时间多 public producer 扩量、account-snapshot/system-alerts/data-maintenance 完整恢复，均移入上线后观察/扩量 backlog。第一版不再为形式化扩量主动触发更多 Binance 公共接口。
...
- [ ] Post-launch staged restoration layer 3a: post-cooldown clean check passed; API queue, cache, sentinel, A/v11 user-stream, and B/v16 user-stream are restored. B listenKey row `501` returned `200`, but B account-state remains stale as intended because no `ACCOUNT_UPDATE` arrived. B/C signed baseline must not be retried. Next: start C/v14 user-stream alone, observe queue/cooldown/journals, then reassess scanner restore gate.
- [ ] Post-launch staged restoration layer 3b: restore A/B/C scanners one at a time after confirming staged drop-ins still keep Kline network disabled and universe small, then restore signed-sensitive account-snapshot only after stricter queue/cooldown checks. P0-B open/close natural exact rows and P0-A longer normal-universe observation remain post-launch evidence collection and must not block first-version skeleton landing.
