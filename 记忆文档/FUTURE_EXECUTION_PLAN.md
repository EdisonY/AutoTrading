...
1. **P0-A Binance API 根治**
   ...
   - 未完成：后续 scanner/market-data fresh-run 暴露新的 `A/v11 GET /fapi/v2/balance` `HTTP 418`，queue `signed:A` cooldown 到 `2026-06-03 04:45:59.656 +08`。scanner/sentinel/market-data-cache 仍未进入新队列+中心状态体系下的谨慎 clean fresh-run。当前 Tencent Testnet 全服务暂停，不应把 paused 状态当作整套 P0-A 最终验收通过。
