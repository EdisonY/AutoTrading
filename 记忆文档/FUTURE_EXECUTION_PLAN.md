...
1. **P0-A Binance API 根治**
   ...
   - 未完成：scanner fresh-run 暴露 public Kline 压力仍可触发 Testnet `HTTP 418/-1003`。最新阻塞为 A/v11 Kline public `HTTP 418/-1003`，queue public cooldown 到 `2026-06-03T10:31:59.888+08:00`；A/B/C scanner、sentinel、market-data-cache 已在 `10:06 CST` 再次停止。下一步必须在 cooldown 过期后按 60s executor 更慢分阶段重试：先单 public producer，稳定后再 A/v11，B/v16、C/v14 最后逐步启动；仍需完整 30 分钟 clean fresh-run。account-snapshot、system-alerts/data-maintenance 仍未进入新体系 fresh-run。
