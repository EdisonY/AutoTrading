...
2. **P0-B Replay/live 同路径**
   ...
   - 已完成：`core.replay` 事件模型、观测型 `ReplayGateResult`、gate audit、OPEN_SKIPPED 归因覆盖；A/v11、B/v16、C/v14 entry-threshold gates 已抽到 `core.strategy_gates` 并由 live scanner 调用；B/v16 15m confirmation gate 也已抽到 shared pure function。
   - 未完成：A/v11 与 C/v14 confirmation/replacement/tail/position/risk/market-data/execution gates 继续抽取、B/v16 risk/position/execution gate 继续共享化、scanner 接入更多纯 gate、同输入 replay/live 结论一致性测试。
