...
- [x] Post-launch A/v11 heartbeat repair: A/v11 scanner liveness is now visible through per-cycle `SCAN_STATS` system events, because `sentinel_scans` only advances for rows with sentinel context and can falsely look stale during cache-only/no-sentinel windows. OKX empty market-data responses are also negative-cached to avoid repeated no-data calls. No strategy parameter or Binance network switch changed.
