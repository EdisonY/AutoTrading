...
- [ ] B/v16 scanner staged validation blocker: first B/v16 small-universe attempt started at `07:55 CST` while cache and sentinel were still active. B/v16 public requests returned `200` at `07:56` and `07:58`, then a B/v16 public request hit `HTTP 418/-1003` at `08:00:27`; public cooldown lasts until `2026-06-04 08:34:51 CST`. B/cache/sentinel are stopped. Retry only after cooldown clears, with cache and sentinel still stopped so B is the single public producer.
- [x] P2-D browser ack API staged validation: Aliyun release `20260604-080630-shadow-c84fefd` deployed the threaded `attention_api_server.py` and restarted `crypto-attention-api.service`. Local `/api/health` and `/api/attention` returned `200`; synthetic `/api/attention/ack` wrote `attention_items` and `attention_acknowledgements`, then test rows were removed and attention JSON re-exported. Remaining public-browser click-through is UI polish/ops check, not first-version API blocker.
...
- [ ] B/v16 scanner staged validation blocker update: B-alone retry after `08:34:51 CST` got two B ticker `200` responses, then the first B Kline `/fapi/v1/klines BTCUSDT 1h limit=200` hit `HTTP 418/-1003`. B stopped and stale deferred Kline request was failed; conservative queue cooldown lasts until `10:08:56 CST`. Commit `d124eeb` is deployed as Tencent no-restart release `20260604-085014-all-d124eeb`; A/B/C staged drop-ins now set `SCANNER_KLINE_NETWORK_ENABLED=0` and `SCANNER_KLINE_CACHE_MAX_AGE_SEC=86400`, with B/C still at `1/1` universe. After cooldown clears, retry B alone to prove scanner skeleton without public Kline REST. Real Kline coverage still waits for P1-E staged Kline backfill/ingest.
...
- [ ] Remaining staged validation P0-A/P0-B: after the public cooldown clears, retry B/v16 alone, then C/v14 alone, under slow queue pacing with no 418/429/-1003; then generate new post-instrumentation rows to prove exact coverage/pass rate. Cache, sentinel, and A/v11 staged gates have passed; old rows without exact cases remain coverage gaps, not accepted parity.
- [ ] Remaining staged validation P1-C/P1-E: run true staged Kline backfill to 30+ day acceptance, staged depth sampling coverage, `--ingest-done`, replay readiness moving from `data_gap`/`waiting_for_samples` to `ready_for_operator_review`, and post-ingest rollout/recovery replay review on real long-history/depth partitions. True queue-priority/market-impact engine is post-launch backlog, not first-version blocker.
- [ ] Remaining staged validation P1-B/P1-D/P2: wait for enough post-refresh samples and review mature PF/cost/failure evidence. Browser attention ack API read/write path is staged-verified; public UI click-through is a lower-risk ops check. Dry-run execution checklist and automation guard exist, but apply/automation remain disabled until explicit policy. Final dirty-data archive/reset and fresh zero-run are allowed only after all staged validation items pass.
