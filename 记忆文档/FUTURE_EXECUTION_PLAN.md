...
- [ ] Post-launch staged restoration layer 3b: restore A/B/C scanners one at a time after confirming staged drop-ins still keep Kline network disabled, market-data fallback disabled, and universe small, then restore account-snapshot in central-only mode after stricter queue/cooldown checks. P0-B open/close natural exact rows and P0-A longer normal-universe observation remain post-launch evidence collection and must not block first-version skeleton landing.
