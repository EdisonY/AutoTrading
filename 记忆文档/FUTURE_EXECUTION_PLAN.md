...
- [ ] Remaining first-version hard gate: final dirty-data archive/reset and fresh zero-run after reset pre-checks show queue/services safe. P0-B open/close natural exact rows and P0-A longer normal-universe observation move to post-launch evidence collection and must not block first-version skeleton landing.
