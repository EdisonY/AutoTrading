...
- [ ] Post-launch staged restoration layer 2: restore sentinel, then A/B/C scanners one at a time, then signed-sensitive account-snapshot only after stricter queue/cooldown checks. P0-B open/close natural exact rows and P0-A longer normal-universe observation remain post-launch evidence collection and must not block first-version skeleton landing.
