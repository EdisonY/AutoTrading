...
2. **P0-B Replay/live 同路径**
   ...
   - 未完成：A/v11 replacement close/open orchestration 还需要更细候选枚举/close-result 证据，A/v11 confirmation/close-result gates、C/v14 remaining market-data/close-result gates、B/v16 remaining position/close-result gates 继续共享化；历史事件驱动的同输入 replay/live 结论一致性测试仍需扩大；多 gate 链目前覆盖 B/v16 threshold、C/v14 tail、A/v11 pool-full replacement 三类路径。没有 exact case 的行只能算 coverage gap，不能算 replay/live parity 已通过。
...
- [ ] Remaining P0-B: expand event-driven same-input replay/live checks beyond single pure gates into orchestration context, especially A/v11 replacement close/open flow and execution-confirmation context.
