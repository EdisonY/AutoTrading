...
- [ ] Post-launch staged restoration layer 3a: per-account account-state baseline is deployed and A/v11 refreshed successfully, but B/v16 baseline hit signed `HTTP 418/-1003`; all Binance-facing services are stopped and global cooldown is active until `2026-06-04 15:15:54.959 +08`. Resume only with read-only queue/service checks after cooldown, then decide whether to retry B later or rely on websocket/account events.
