...
- [x] B/C cache-only staged retry after `10:08:56 CST`: queue was clean before start (`active_cooldowns=[]`, `pending=[]`). B/v16 ran alone with cache/sentinel/A/C stopped and produced two fresh `/fapi/v1/ticker/24hr` `200` rows, no Kline REST, no pending backlog, and no cooldown. C/v14 then ran alone and produced two fresh `/fapi/v1/ticker/24hr` `200` rows, no Kline REST, no pending backlog, and no cooldown. Both scanners were stopped after the evidence window. `SCANNER_MARKET_CACHE_MAX_AGE_SEC` is now added locally so the next staged run can reuse older market cache and avoid ticker calls just to choose the staged universe.
...
- [ ] Remaining staged validation P0-A/P0-B: B/v16 and C/v14 cache-only safety windows are clean, but they produced only `SYSTEM/SCAN_STATS`; no fresh open-flow or scan-level exact gate rows were created. Continue with a low-API staged run after deploying `SCANNER_MARKET_CACHE_MAX_AGE_SEC`, then generate post-instrumentation exact rows to prove coverage/pass rate. Old rows without exact cases remain coverage gaps, not accepted parity.
