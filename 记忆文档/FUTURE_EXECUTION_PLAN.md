...
2. **P0-B Replay/live 同路径**
   ...
   - 未完成：A/v11 replacement close/open orchestration 还需要更细候选枚举证据，success-path close/confirmation context 仍需更多 exact case，历史事件驱动的同输入 replay/live 结论一致性测试仍需扩大；多 gate 链目前覆盖 B/v16 threshold、C/v14 tail、A/v11 pool-full replacement 和 close failure execution-result paths。没有 exact case 的行只能算 coverage gap，不能算 replay/live parity 已通过。
