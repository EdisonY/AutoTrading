...
- [ ] P2-D browser ack staged validation: Aliyun `crypto-attention-api.service` was active/listening, but `/api/health` and `/api/attention` timed out. Local fix switches to threaded HTTP serving with request backlog `64`; deploy/restart and endpoint verification remain before marking browser ack complete.
