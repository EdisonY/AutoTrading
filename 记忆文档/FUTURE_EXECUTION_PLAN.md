...
- [ ] Remaining P1-B/P1-D: wait for enough post-refresh samples, then review full paper fill/slippage simulation on real long-history/depth partitions, mature PF/cost/failure evidence, and an explicitly approved automatic rollback procedure before any automation can run. Dry-run execution checklist now exists, but apply/automation remain disabled.
