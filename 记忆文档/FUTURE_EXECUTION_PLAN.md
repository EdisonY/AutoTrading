...
- [ ] B/v16 scanner staged validation blocker update: B-alone retry after `08:34:51 CST` got two B ticker `200` responses, then the first B Kline `/fapi/v1/klines BTCUSDT 1h limit=200` hit `HTTP 418/-1003`. B stopped and stale deferred Kline request was failed; conservative queue cooldown lasts until `10:08:56 CST`. Next retry should use the new `SCANNER_KLINE_NETWORK_ENABLED=0` staged cache-only path, proving scanner skeleton without public Kline REST. Real Kline coverage still waits for P1-E staged Kline backfill/ingest.
