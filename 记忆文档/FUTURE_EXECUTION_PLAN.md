...
2. **P0-B Replay/live 同路径**
   ...
   - 已完成：`core.replay` 事件模型、观测型 `ReplayGateResult`、gate audit、OPEN_SKIPPED 归因覆盖；B/v16 entry-threshold gate 与 15m confirmation gate 已抽到 `core.strategy_gates` 并由 live scanner 调用。
   - 未完成：A/v11 与 C/v14 纯 gate 抽取、B/v16 risk/position/execution gate 继续共享化、scanner 接入更多纯 gate、同输入 replay/live 结论一致性测试。
