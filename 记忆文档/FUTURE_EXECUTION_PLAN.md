...
- [ ] Remaining first-version hard gate: fresh zero-run after signed cooldown clears. The first post-reset attempt hit A/v11 signed `/fapi/v2/positionRisk` `HTTP 418/-1003`; all Binance-facing services were stopped and retry must wait until at least `2026-06-04T13:04:53.840+08:00` plus a clean read-only queue/service check. P0-B open/close natural exact rows and P0-A longer normal-universe observation remain post-launch evidence collection and must not block first-version skeleton landing.
