...
2026-06-04 骨架验收规则补充：第一版先看骨头，不扩肉。`long_term_skeleton_review.py` 本地 strict 检查 143/143 bones；远端 Tencent/Aliyun 扁平 release root 只负责展示/施工态验收，路径通过 `部署工具/ -> root`、`策略文件/ -> root`、`交易客户端/ -> root`、`部署工具/systemd/ -> systemd/` 解析，未部署到 runtime 节点的源码/测试骨架按本地 strict 验收结果视为 repo-only bone。所有优化项继续写入 post-launch backlog，不作为第一版阻塞。
...
- [ ] Remaining staged validation P0-A/P0-B: deploy/sync skeleton matrix chain, then run staged cache -> sentinel -> A/v11 -> B/v16 -> C/v14 fresh-run under slow queue pacing with no 418/429/-1003, and generate new post-instrumentation rows to prove exact coverage/pass rate. Old rows without exact cases remain coverage gaps, not accepted parity.
