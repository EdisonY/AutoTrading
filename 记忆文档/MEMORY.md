...
## 2026-05-27 未来策略优化执行计划固化
- 用户提供 `C:\Users\Eels\Desktop\1.txt` 作为当前系统总结，并要求判断正确/不对处、补充理解，再把详细计划落为未来可执行计划。
- 核心判断：文档方向基本正确，尤其恢复仓陷阱、B/v16 当前最可信、C/v14 噪声大、阶段过滤不宜一刀切；但需要修正口径混用，包括主动策略 PnL、恢复仓 PnL、当前浮盈、反事实 PnL、影子实验 PnL不能混在一起。
- 已新增 `记忆文档/FUTURE_EXECUTION_PLAN.md`。该计划明确能解决的系统性问题：口径混乱、证据不足、策略优先级不清、哨兵贡献未量化、恢复仓误计 alpha、进化门禁还不够硬；同时说明它不能保证解决所有未来市场变化。
- 未来执行顺序：策略真实质量账本 -> 总入口质量板 -> 哨兵贡献评估 -> A/v11 与 B/v16 影子实验 -> C/v14 重构/退役实验 -> 恢复仓管理研究 -> 进化门禁强化。任何实盘参数调整必须先经过计划中的证据链。
