...
- 2026-06-01：腾讯账号快照出现 Binance testnet `HTTP 418 / -1003 Way too many requests`，曾导致入口页拉到误导性 0 仓快照。已将账号快照改为 API 错误时不覆盖最新有效快照，并把错误升级到系统告警；后续仍需继续做 API budget/websocket 化。
