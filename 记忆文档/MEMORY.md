...
- N2/N3 又推进一步：`counterfactual_open_skips.py` 已开始通过 `core.replay` 归一 OPEN_SKIPPED 并把 `replay_gate` 写入反事实 JSON/DB payload；`portal_dashboard.py` 已把研究仓摘要接入入口页，用于快速看样本漏斗、主要否决层和 DuckDB/Parquet 是否新鲜。剩余重点是把 A/B/C 实盘门控抽成纯函数、让 replay 与 live 真正同路径。
