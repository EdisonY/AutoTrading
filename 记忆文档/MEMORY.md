...
- 2026-06-03 05:43-06:42 CST：P0-A 继续 fresh-run，暴露并修复新 public 阻塞。queue + A/B/C user-stream-only 观察通过后，market-data-cache 与 sentinel 可在中心队列下 clean 运行；A/v11 单 scanner 短观察也能让 exchangeInfo/ticker 走 `200`。但 scanner Kline 压力仍触发 Testnet public `HTTP 418/-1003`，第二轮加 B/v16 后 public cooldown 延到 `2026-06-03 07:18:57 +08`。已停 A/B/C scanner、sentinel、market-data-cache，保留 API queue + A/B/C user-stream。已新增 `core.binance_api_queue_client` 前置 cooldown check，active cooldown 时直接返回 `queue_status=cooldown`，不再入队；client timeout 会把未 leased 的 queued/deferred 请求标 failed，避免晚到重放；`crypto-binance-api-queue.service` 施工期 executor 从 `--interval 5` 降为 `--interval 15`；A/B/C/sentinel/market-data public queue waits 至少 `60s`。阻塞项预计 `07:18:57 +08` 后可修复重试，但下一轮要么进一步降到约 `60s` executor，要么先把 scanner Kline 读取更多转到本地/cache，仍要求 30 分钟无 418/429/-1003。
