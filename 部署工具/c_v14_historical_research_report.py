"""C/v14 one-year historical research report.

Read-only Tencent research runner. It compares a small pre-registered C/v14
...
def latest_json_path(root: Path = ROOT) -> Path:
    return shared.runtime_dir(root) / "c_v14_historical_research_latest.json"


def latest_html_path(root: Path = ROOT) -> Path:
    return shared.reports_dir(root) / "c_v14_historical_research_latest.html"
...
def default_c_v14_params(interval: str) -> dict[str, float]:
    """Baseline approximates current C/v14 live-approved parameters.

    Live C uses 1h entry + 15m confirmation. The research adapter can run other
    intervals for comparison, so 30m/4h reuse the 1h score baseline as a
    reference only.
    """

    long_score_threshold = 60.0 if interval == "15m" else 50.0
    return {
        "long_score_threshold": long_score_threshold,
        "short_score_threshold": long_score_threshold,
        "score_max": 85.0,
...
def build_variants(interval: str, max_variants: int) -> list[dict[str, Any]]:
    base = default_c_v14_params(interval)
    raw: list[tuple[str, dict[str, float]]] = [("baseline_live_approx", dict(base))]

    threshold_candidates = [55.0, 60.0, 65.0, 70.0] if interval == "15m" else [45.0, 50.0, 55.0, 60.0]
    for value in threshold_candidates:
        params = dict(base)
        params["long_score_threshold"] = value
        params["short_score_threshold"] = value
        raw.append((f"score_threshold={value:g}", params))

    for value in [75.0, 80.0, 90.0, 100.0]:
        params = dict(base)
        params["score_max"] = value
        raw.append((f"score_max={value:g}", params))
    ...
    combo_rows = (
        [(55.0, 80.0, 2.0), (60.0, 80.0, 2.0), (65.0, 90.0, 2.5), (70.0, 90.0, 1.5)]
        if interval == "15m"
        else [(45.0, 80.0, 2.0), (50.0, 80.0, 2.5), (55.0, 90.0, 1.5), (60.0, 90.0, 2.0)]
    )
    for threshold, cap, stop in combo_rows:
        params = dict(base)
        params["long_score_threshold"] = threshold
        params["short_score_threshold"] = threshold
        params["score_max"] = cap
...
def run_research(
    *,
    ...
    for interval in intervals:
        ...
        baseline_params = baseline.get("params") if isinstance(baseline.get("params"), dict) else default_c_v14_params(interval)
    ...
    *,
    ...
    payload = {
        ...
        "config": {
            ...
            "baseline_params": {interval: default_c_v14_params(interval) for interval in intervals},
            "tested_parameters": ["long_score_threshold", "short_score_threshold", "score_max", "atr_stop_multiplier"],
            "not_tested_parameters": ["btc_trend_filter"],
            "not_tested_reason": "btc_trend_filter is registered, but this one-year warehouse has Kline bars only; no aligned one-year BTC trend feature series is available in the research adapter.",
            "live_timeframe_note": "Current C/v14 live entry is 1h with 15m confirmation. 15m/30m/4h rows here are research comparisons, not proof those live entry timeframes are enabled.",
            "atr_stop_note": "Live C/v14 uses ATR stop bands by volatility regime; this adapter tests fixed atr_stop_multiplier values only.",
        },
...
def operator_summary(interval_results: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "warnings": warnings,
        "plain_advice": (
            "只把通过 OOS/邻近稳定性初筛的参数作为下一轮人工研究候选；不要把单窗口最好曲线直接写入 C/v14 实盘配置。"
        ),
        "auto_apply_allowed": False,
        "automatic_upgrade_allowed": False,
    }
...
def params_text(params: Any) -> str:
    ...
    selected = ["long_score_threshold", "score_max", "atr_stop_multiplier", "trailing_pullback_atr", "tp_mult"]
...
<main>
  ...
  <section>
    <h2>安全边界</h2>
    <p>本报告只读 Tencent 一年历史 K线仓；不调用 Binance；不发订单；不改 A/B/C 扫描频率；不改 <code>config/v14.toml</code>；不触发自动调参、自动回滚、自动升级。</p>
    <p>本轮只测试 <code>long_score_threshold</code>、<code>short_score_threshold</code>、<code>score_max</code>、<code>atr_stop_multiplier</code>。<code>btc_trend_filter</code> 已登记但当前一年仓没有对齐的一年期 BTC 趋势特征序列，因此不拿它做调参依据，避免假拟合。</p>
    <p>当前 C/v14 live 主路径是 1h 入场 + 15m 确认；本报告的 15m/30m/4h 是横向研究，不代表这些周期已进入实盘入场路径。</p>
  </section>
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run C/v14 historical research report")
